...
class NoticeProvider with ChangeNotifier {
  ...
  NoticeItem? get homeNotice {
    ...
    try {
      ...
      return _notices.first; // 고정된 게 없으면 최신 공지
    }
  }
  ...
  // 💡 공지사항 이미지 업로드 로직 이관
  Future<List<String>> uploadNoticeImages(List<XFile> images) async {
    List<String> urls = [];
  ...
  Future<void> addNotice(String title, String content, {List<String> imageUrls = const []}) async {
    ...
    await _db.collection('notices').add(newItem.toMap());
  }
  ...
  Future<void> pinNotice(String id) async {
    ...
    for (var n in _notices) {
      if (n.isPinned) batch.update(_db.collection('notices').doc(n.id), {'isPinned': false});
    }
    batch.update(_db.collection('notices').doc(id), {'isPinned': true});
    await batch.commit();
  }

  // 💡 알림 발송용 서비스 계정 (FCM 로직 이관)
  ...
  Future<void> sendNoticePush(String title, String content) async {
    try {
      ...
      for (var doc in tokenSnapshot.docs) {
        ...
        await http.post(
          ...
          body: jsonEncode({
            'message': {
              'token': token,
              'notification': {'title': '[KUST 공지] $title', 'body': content},
